...
class Fixer {
    ...
    public function propose_plan(array $issues){
        ...
        foreach($issues as $i){
            switch($i['type']){
                ...
                case 'db': 
                    $actions[]=['action'=>'cleanup_orphan_postmeta','safety'=>'safe']; 
                    $actions[]=['action'=>'cleanup_orphan_termmeta','safety'=>'safe'];
                break;
                case 'perf': $actions[]=['action'=>'analyze_autoload_bloat','safety'=>'review']; break;
                case 'crawler': $actions[]=['action'=>'report_broken_links','safety'=>'safe']; break;
            }
        }
    ...
    public function rest_apply(\WP_REST_Request $req){
        ...
        $opts = Plugin::instance()->opts; $dry=!empty($opts['safety']['dry_run']); $results=[];
        foreach($actions as $a){ $m='do_'.$a['action']; if(method_exists($this,$m)) $results[]=['action'=>$a,'result'=>$dry?'DRY-RUN':call_user_func([$this,$m],$a)]; }
        return ['applied'=>!$dry,'results'=>$results];
    }
    ...
    private function do_cleanup_orphan_termmeta(){ global $wpdb; $cnt=$wpdb->query("DELETE tm FROM {$wpdb->termmeta} tm LEFT JOIN {$wpdb->terms} t ON t.term_id=tm.term_id WHERE t.term_id IS NULL"); return "Deleted {$cnt} orphaned termmeta rows."; }
    ...
    private function do_report_broken_links(){ $s = Crawler::instance()->peek_summary(); return $s ?: ['checked'=>0,'broken'=>0,'samples'=>[]]; }
}
