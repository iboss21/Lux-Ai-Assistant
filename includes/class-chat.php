...
class Chat {
    ...
    public function rest_chat(WP_REST_Request $req){
        ...
        if (!\LuxAI\Utils\Rate_Limit::check('chat', get_current_user_id(), $opts['rate_limiting']??[]))
        ...
        if (!\LuxAI\Utils\Cost_Guard::allow_send('chat')){
            return new WP_Error('luxai_cost_cap','Daily cost cap reached',['status'=>429]);
        }
        ...
        $estimated_tokens = max(1, intval(strlen($message)/4) + intval(strlen($reply)/4));
        \LuxAI\Utils\Cost_Guard::add_usage($prov instanceof Provider_OpenAI ? 'openai' : 'anthropic', $estimated_tokens);

        \LuxAI\Utils\Logger::info('chat','assistant','Chat reply generated');
        return ['reply'=>$reply];
    }
}
