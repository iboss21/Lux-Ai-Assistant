...
class Plugin {
    ...
    public function register_rest(){
        ...
        register_rest_route('luxai/v1','/crawl/run',[
            'methods'=>'POST','permission_callback'=>function(){return current_user_can($this->cap_manage());},
            'callback'=>[Crawler::instance(),'rest_run'],
            'args'=>['limit'=>['type'=>'integer','required'=>false]]
        ]);
    }

    public function register_site_health($tests){
        $tests['direct']['luxai_audit_summary'] = ['label'=>__('LuxAI Audit Summary','luxai'),'test'=>[Audit::instance(),'site_health_test']];
        return $tests;
    }
}
